...
<div class="container">
	<div class="row">
		...
		<div class="col-sm-7 col-12">
			<input type="text" class="select_medicine SearchMedicine_search_box form-control" placeholder="<?= $msg_show?>" tabindex="1" placeholder="Search">
		</div>
	</div>
	<div class="row">
		<div class="col-sm-3"></div>
		<div class="col-sm-7 col-12">
			<span class="search_medicine_result searchpagescrolling2" style="margin-top:30px;"></span>
		</div>
	</div>
</div>
	<div class="row">
					...
					<tr class="remove_css_<?= $i ?>">
						...
						<td class="import_order_td_<?= $i ?>">
							<div class="row">
								<div class="col-sm-1">
									Old Image
									<img src="<?=constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/logo2.png" width="64px;" style="margin-top: 5px;" class="image_css_<?= $i ?>" alt="">
								</div>
								<div class="col-sm-1">
									New Image
									<img src="<?=constant('img_url_site'); ?>new_pix/<?php echo $row->image1 ?>" width="64px;" style="margin-top: 5px;" class="new_image_css_<?= $i ?>" alt=""><br>
								</div>
								<div class="col-sm-10">
									<div class="row">
										<div class="col-sm-12">
											...
											<img src="<?= constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/delete_icon.png" width="18px;" alt="Delete medicine"> Delete medicine </a> </span> | <span class="selected_SearchAnotherMedicine_<?= $i ?>" style="display:none"><a href="javascript:Search_Another_Medicine_js('<?= $i ?>')" class="delete_all" title="Change medicine"><img src="<?= constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/edit_icon.png" width="18px;" alt="Change Medicine"> Change medicine</a></span>
										</div>
									...
									<div class="row select_product_<?= $i ?>" style="display:none">
										...
										<div class="col-sm-12">
											...
											By : <span class="selected_company_full_name_<?= $i ?>"></span></span> | <span class="selected_msg_<?= $i ?>"> Loading.... </span><span class="selected_suggest_<?= $i ?>" style="display:none"> | <a href="javascript:delete_suggest('<?= $i ?>')" title="Delete suggested medicine" class="delete_all"><img src="<?= constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/delete_icon.png" width="18px;" alt="Delete suggested medicine">Delete suggested medicine</a> </span> | <a href="javascript:show_big_img_div(<?= $i ?>)"
											title="Delete suggested medicine" class="delete_all"><img src="<?= constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/edit_icon.png" width="18px;" alt="Edit Image"> Edit Image </a> | <a href="javascript:show_big_des_div(<?= $i ?>)"
											title="Delete suggested medicine" class="delete_all"><img src="<?= constant('img_url_site'); ?>img_v<?= constant('site_v') ?>/edit_icon.png" width="18px;" alt="Edit Description"> Edit Description</a>
		...
		<div class="col-sm-6 col-6 text-right">
			<a href="<?= base_url(); ?>admin/manage_image_scraping">
...
function import_order_dropdownbox(cssid)
{
	...
	$.ajax({
		...
		url        : "<?= base_url(); ?>admin/manage_image_scraping/import_order_dropdownbox",
		cache	   : true,
		error: function(){
	...
	swal({
		...
		if (result) {
			$.ajax({
				...
				url        : "<?= base_url(); ?>admin/manage_image_scraping/remove_row",
				cache	   : false,
				error: function(){
...
function delete_suggest(cssid)
{
	...
	swal({
		...
		if (result) {
			$.ajax({
				url: "<?php echo base_url(); ?>admin/manage_image_scraping/delete_suggest",
				type:"POST",
				/*dataType: 'html',*/
...
{	
	...
	if(new_i!="")
	{
		...
		$.ajax({
			url: "<?= base_url(); ?>admin/manage_image_scraping/select_medicine_in_search_box",
			type:"POST",
			/*dataType: 'html',*/
...
function copy_old_image() {
  ...
  $.ajax({
		url: "<?= base_url(); ?>admin/manage_image_scraping/update_copy_old_image",
		type:"POST",
		/*dataType: 'html',*/
...
function save_big_des_div() {
  ...
  $.ajax({
		url: "<?= base_url(); ?>admin/manage_image_scraping/update_description",
		type:"POST",
		/*dataType: 'html',*/
...
<script src="<?= constant('img_url_site'); ?>assets/website/select_css/chosen.jquery.js" type="text/javascript"></script>
<script src="<?= constant('img_url_site'); ?>assets/website/select_css/init.js" type="text/javascript" charset="utf-8"></script>