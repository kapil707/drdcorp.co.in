...
class Manage_image_scraping extends CI_Controller {

	var $Page_title = "Manage Image Scraping";
	var $Page_name  = "manage_image_scraping";
	var $Page_view  = "manage_image_scraping";
	var $Page_menu  = "manage_image_scraping";
	var $page_controllers = "manage_image_scraping";
	...
	public function edit($id)
	{
		...
		$update_image1 = $update_image2 = $update_image3 = $update_image4 = 0;
		extract($_POST);
		if(isset($Submit))
		{
			$itemid = $i_code;
			...
			{
				$image = $old_image1;
				$update_image1 = $old_update_image1;
			}
			...
			{
				$image2 = $old_image2;
				$update_image2 = $old_update_image2;
			}
			...
			{
				$image3 = $old_image3;
				$update_image3 = $old_update_image3;
			}
			...
			{
				$image4 = $old_image4;
				$update_image4 = $old_update_image4;
			}
			
			$result = "";
			$dt = array(
				'image1'=>$this->new_clean($image),
				'image2'=>$this->new_clean($image2),
				'image3'=>$this->new_clean($image3),
				'image4'=>$this->new_clean($image4),
				'itemintro1'=>$this->new_clean($itemintro1),
				'itemintro2'=>$this->new_clean($itemintro2),
				'update_image1'=>$update_image1,
				'update_image2'=>$update_image2,
				'update_image3'=>$update_image3,
				'update_image4'=>$update_image4,
				'featured'=>$featured,
			);
	...
	public function upload_import_file(){
		...
		if(move_uploaded_file($uploadedfile, $upload_path.$filename))
		{
				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
				{
					...
					for ($row=$headername; $row<=$highestRow; $row++)
					{
						...
						if($item_name!="")
						{
							...
							$itemintro1_dt = $this->new_clean($itemintro1_dt);
							$itemintro2_dt = $this->new_clean($itemintro2_dt);
							$image1_dt = $this->new_clean($image1_dt);
							$image2_dt = $this->new_clean($image2_dt);
							$image3_dt = $this->new_clean($image3_dt);
							$image4_dt = $this->new_clean($image4_dt);
			...
			$url = base_url()."admin/manage_image_scraping/search/$order_id";
		}
		else{
			$url = base_url()."admin/manage_image_scraping";
		}
$items.= <<<EOD
{"url":"{$url}"},
EOD;
...
{"items":[<?= $items;?>]}<?php
	...
	public function search($order_id='')
	{
		...
		if(empty($result))
		{
			redirect(base_url()."manage_image_scraping");
		}
	...
	public function import_order_dropdownbox()
	{
		...
		$order_item_name	= $keyword 	= $this->new_clean($_POST["item_name"]);
	...
	{		
		...
		if(!empty($row->id))
		{
			$item_name_dt  = $this->new_clean($row->item_name);
			$itemintro1_dt = $this->new_clean($row->itemintro1);
			$itemintro2_dt = $this->new_clean($row->itemintro2);
			$image1_dt = "new_pix/".$this->new_clean($row->image1);
			$image2_dt = "new_pix/".$this->new_clean($row->image2);
			$image3_dt = "new_pix/".$this->new_clean($row->image3);
			$image4_dt = "new_pix/".$this->new_clean($row->image4);
...
{"items":[<?= $items;?>]}<?php
	...
	public function select_medicine_in_search_box()
	{
		...
		$your_item_name = $this->new_clean($_POST["your_item_name"]);
		$item_name		= $this->new_clean($_POST["item_name"]);
...
{"items":[<?= $items;?>]}<?php
	...
	public function delete_suggest()
	{
		...
		$your_item_name = $this->new_clean($_POST["item_name"]);
		$where = array('your_item_name'=>$your_item_name);
		$this->Scheme_Model->delete_fun("drd_import_orders_suggest",$where);
...
{"items":[<?= $items;?>]}<?php
	...
	public function update_description()
	{
		...
		$itemintro1_dt = $this->new_clean($new_description1);
		$itemintro2_dt = $this->new_clean($new_description2);
...
{"items":[<?= $items;?>]}<?php
	...
	public function update_copy_old_image()
	{
		...
		$items = "";
		$id = $_POST["id"];
		$css_new_img_01 = $this->new_clean($_POST["css_new_img_01"]);
		$css_new_img_02 = $this->new_clean($_POST["css_new_img_02"]);
		$css_new_img_03 = $this->new_clean($_POST["css_new_img_03"]);
		$css_new_img_04 = $this->new_clean($_POST["css_new_img_04"]);
...
{"items":[<?= $items;?>]}<?php
	...
	public function select_medicine_in_search_box1()
	{
		//error_reporting(0);
		header('Content-Type: application/json');
		$items = "";
		...
		$this->db->where('id',$id);
		$row = $this->db->get($Page_tbl)->row();
		if(!empty($row->id))
		{
			$where = array('id'=>$id,);					
			$dt = array('selected_item_name'=>$item_name);
			$this->Scheme_Model->edit_fun($Page_tbl,$dt,$where);
			$response = 1;			
		}
