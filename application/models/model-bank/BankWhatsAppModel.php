		...
		foreach($result as $row) {
			...
			if (in_array($whatsapp_body, $parts)){
				...
				foreach($parts as $from_text_find_chemist_new) {
					...
					if(empty($whatsapp_chemist)){
						echo "SELECT (SELECT body FROM tbl_whatsapp_message WHERE from_number = wm.from_number AND REPLACE(REPLACE(REPLACE(TRIM(body), ' ', ''), '\n', ''), '\r', '') = '$from_text_find_chemist_new' AND FROM_UNIXTIME(timestamp) BETWEEN DATE_SUB(FROM_UNIXTIME(wm.timestamp), INTERVAL 7 MINUTE) AND DATE_ADD(FROM_UNIXTIME(wm.timestamp), INTERVAL 7 MINUTE) LIMIT 1) AS text FROM tbl_whatsapp_message AS wm WHERE wm.id = $whatsapp_id";
						$row1 = $this->BankModel->select_query("SELECT (SELECT body FROM tbl_whatsapp_message WHERE from_number = wm.from_number AND REPLACE(REPLACE(REPLACE(TRIM(body), ' ', ''), '\n', ''), '\r', '') = '$from_text_find_chemist_new' AND FROM_UNIXTIME(timestamp) BETWEEN DATE_SUB(FROM_UNIXTIME(wm.timestamp), INTERVAL 7 MINUTE) AND DATE_ADD(FROM_UNIXTIME(wm.timestamp), INTERVAL 7 MINUTE) LIMIT 1) AS text FROM tbl_whatsapp_message AS wm WHERE wm.id = $whatsapp_id");
						$row1 = $row1->row();
						if(!empty($row1)){
							$text = trim($row1->text);
							if(!empty($text) && !empty($from_text_find_chemist_new)){
